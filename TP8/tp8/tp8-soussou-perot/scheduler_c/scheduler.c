...
void Scheduler__select_aux_step(Scheduler_data__task_status ts,
  ...
  int v_1;
  int v;
  v_3 = (ts.current_deadline<acc.dline);
  v_1 = (cpu==Scheduler_data__nocpu);
  v = (ts.status==Scheduler_data__Ready);
  v_5.tid = tid;
  v_5.dline = ts.current_deadline;
  v_2 = (v&&v_1);
  v_4 = (v_2&&v_3);
  if (v_4) {
    _out->acc_o = v_5;
...
void Scheduler__scheduler_step(Scheduler_data__scheduler_state si,
  ...
  _out->so.current_date = d;
  {
    int _6;
    for (_6 = 0; _6 < 5; ++_6) {
      _out->so.cpu[_6] = cpu1[_6];
    }
  };
  {
    int _5;
    for (_5 = 0; _5 < 5; ++_5) {
      _out->so.tasks[_5] = t_c1[_5];
    }
  };;
}
...
void Scheduler__main_reset(Scheduler__main_mem* self) {
  {
    int _8;
    for (_8 = 0; _8 < 5; ++_8) {
      self->st.tasks[_8].left = 0;
      self->st.tasks[_8].current_deadline = 0;
      self->st.tasks[_8].status = Scheduler_data__Waiting;
    }
  };
  {
    int _7;
